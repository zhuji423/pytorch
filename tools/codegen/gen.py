...
from tools.jit.gen_unboxing_wrappers import ComputeUnboxingWrapper
...
class FileManager:
    ...
    def write_with_template(self, filename: str, template_fn: str,
                            env_callable: Callable[[], Union[str, Dict[str, Any]]], template_dir_override: str = None,) -> None:
        ...
        if not self.dry_run:
            ...
            if isinstance(env, dict):
                ...
                template = _read_template(os.path.join(template_dir_override if template_dir_override else self.template_dir, template_fn))
...
def main() -> None:
    ...
    parser.add_argument(
        '--enable_codegen_unboxing',
        action='store_true',
        help='enable codegen unboxing which generates CodegenFunctions.h/.cpp '
             'and CodegenUnboxingWrappers.cpp. It can be used to replace existing '
             'template-based unboxing and can work with static dispatch together '
             'to provide a lightweight operator registration & dispatch system.')
